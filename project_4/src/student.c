 ...
 * in this function and/or the dequeue function to pick the process with the smallest priority.
 * 
 *
...
{    
    ...
    if (is_empty(queue)) {
        ...
        if(scheduler_algorithm == FCFS) {
            pcb_t* cur = queue->head;
            pcb_t* prev = NULL; 

            while (cur != NULL && (cur->arrival_time < process->arrival_time)) {
                prev = cur;
                cur = cur->next;
            }
            if(prev == NULL) {
            ...
            pcb_t* cur = queue->head;
            pcb_t* prev = NULL; 
            while (cur != NULL && (cur->priority < process->priority)) {
                prev = cur;
                cur = cur->next;
            }
            if(prev == NULL) {
            ...
            pcb_t* cur = queue->head;
            pcb_t* prev = NULL; 
            while (cur != NULL && (cur->total_time_remaining < process->total_time_remaining)) {
                prev = cur;
                cur = cur->next;
            }
            if(prev == NULL) {
 ...
 *
 * NOTE: For Priority, FCFS, and SRTF scheduling, you will need to have additional logic
 * in this function and/or the enqueue function to pick the process with the smallest priority.
 * 
 * 
 * @param queue pointer to the ready queue
 */
...
static void schedule(unsigned int cpu_id)
{
    ...
    context_switch(cpu_id, first, timeslice);
}
...
extern void yield(unsigned int cpu_id)
{
    ...
    current[cpu_id] = NULL; 
    pthread_mutex_unlock(&current_mutex); //might need to move this mutex?
...
extern void terminate(unsigned int cpu_id)
{
    ...
    current[cpu_id] = NULL; // is this necessary?
    pthread_mutex_unlock(&current_mutex);
    process->state = PROCESS_TERMINATED;

    schedule(cpu_id); // might also try handling mutex outside of schedule func, but for now going to handle within
}
 ...
 * 
 * We've provided an API for marking a CPU for preemption via `mark_for_preemption(unsigned int cpu_id)`
 * 
...
extern void wake_up(pcb_t *process)
{
    ...
    if(scheduler_algorithm == PRIORITY || scheduler_algorithm == SRTF) {
        ...
        unsigned int process_val = (scheduler_algorithm == PRIORITY) ? process->priority : process->total_time_remaining;
        unsigned int m = 0;
        for (size_t i = 0; i < cpu_count; i++) {
            unsigned int cur_val = (scheduler_algorithm == PRIORITY) ? current[i]->priority : current[i]->total_time_remaining;
            unsigned int max_val = (scheduler_algorithm == PRIORITY) ? current[m]->priority : current[m]->total_time_remaining;
            if (cur_val > max_val) {
                m = i;
            }
        }
        ...
        unsigned int max_val = (scheduler_algorithm == PRIORITY) ? current[m]->priority : current[m]->total_time_remaining;
        if(max_val > process_val) {
            pthread_mutex_unlock(&current_mutex);
            mark_for_preemption((unsigned int)m);
        } else {
            pthread_mutex_unlock(&current_mutex);
        }
    }
