...
#define IRWL_READER_LOCK(i)                                                    \
  ...
  while ((i).writers > 0) {                                                    \
    __real_pthread_cond_wait(&(i).no_writers, &(i).mutex);                     \
  }
...
void __wrap_pthread_cond_wait(pthread_cond_t *cond, pthread_mutex_t *mutex) {
  // Release IRWL first
  IRWL_WRITER_UNLOCK(student_lock);

  // normal pthread_cond_wait
  __real_pthread_cond_wait(cond, mutex);
  ...
  while (1) {
    ...
    if (simulator_cpu_data[cpu_id].current == NULL) {
      ...
      while (simulator_cpu_data[cpu_id].state == CPU_RUNNING)
        __real_pthread_cond_wait(&simulator_cpu_data[cpu_id].wakeup,
                                 &simulator_mutex);
    }
...
/*
 * context_switch() and mark_for_preemption() are the two functions available to
 * student's code.
 */
...
extern void mark_for_preemption(unsigned int cpu_id) {
  assert(cpu_id < cpu_count);
  ...
  /*
   * It is possible that the student's code calls mark_for_preemption() at the
...
static void simulate_process(unsigned int cpu_id, pcb_t *pcb) {
  ...
  case OP_CPU:
    ...
    if (pc->time > 0) {
      ...
      pcb->time_in_CPU_burst =
          pc->time--; // Set remaining time then decrement time
      pcb->total_time_remaining--;
...
static void simulate_io(void) {
  ...
  if (io_queue_head->execution_time-- <= 0) {
    ...
    io_queue_head->pcb->total_time_remaining--;
  }
}
...
/* get_current_time() returns the current simulation time and is thread-safe */
extern unsigned int get_current_time(void) {
  pthread_mutex_lock(&simulator_mutex);
  unsigned int time = simulator_time;
  pthread_mutex_unlock(&simulator_mutex);
  return time;
}
